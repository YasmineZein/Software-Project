...
app.post("/video",admin, upload.single ('uploadedFile'),async(req,res)=>{
    try{
    const {Name,Type} =req.body;
    if(!Name || !Type)
        return res.status(404).json({msg:"need data"});
...
app.post("/editvideo",admin,async(req,res)=>{
    try{
...
app.post("/edit",admin,async(req,res)=>{
  ...
  // upload routing
  app.post ('/upload',admin, upload.single ('uploadedFile'), (req, res) => {
    const upload=Upload.create({
        Name:req.body.fileTitle,
        url:req.file.filename
    })
...
app.post("/upload/delete",admin,async(req,res)=>{
    try{
    const {_id} =req.body;
...
app.post("/upload/edit",admin,async(req,res)=>{
    try{
