...
export default function Users(props) {
  ...
  const verifycode=async(_id)=>{
    ...
    setloading(false);
    Axios.post( 
      BACKEND_DOMAIN+`/api/auth/makeadmin`,
      inputs,
      config
    ).then((response)=>{
      setloading(true);
  ...
  const editcode=async(_id,Name,Phone,Money,Email)=>{
      setInputs2((prev)=>({
        _id:_id,
        Name:Name,
        Phone:Phone,
                ...
                {students.map((student,i)=>{
                  return(<tr>
                          ...
                          <td>{//_id,Name,Phone,Year,Money,Email
                            ...
                            <i className="fa-solid fa-pen" onClick={()=>editcode(student._id,student.UserName,student.Phone,student.Money,student.Email)}></i>
