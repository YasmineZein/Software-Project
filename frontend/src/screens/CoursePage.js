...
export default function CoursePage() {
  ...
  const buysection=()=>{
    ...
    setloading(false);
    Axios.get( 
      BACKEND_DOMAIN+`/api/course/buy/${id}`,
      config
    ).then((response)=>{
      setloading(true);
  ...
  return (
    ...
    <p style={{"textAlign":"right","margin":"0 0 20px 0"}}>إعداد {course.Instructor&&course.Instructor.Name}</p>
    <img src={course&&BACKEND_DOMAIN+course.Img} alt='course img' style={{"width":"100%"}}/>
    {
      course.Have?<Link to={`/course/${id}/section/${course.Sections[0]._id}`}><button>ابداء</button></Link>:<button onClick={buysection}>السعر {course.Price}</button>
    }

    {
      course.Sections&&course.Sections.map(s=><div className='section'>
         <div className='title'>
          {s.Name}
          {s.Isfree?<Link to={`/course/${id}/section/${s._id}`}><button>حصة مجانية</button></Link>:course.Have?<Link to={`/course/${id}/section/${s._id}`}><button>ابداء</button></Link>:""}
        </div>
        {
