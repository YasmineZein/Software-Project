...
export default function SectionPage() {
  ...
  useEffect(() => {
    ...
    setloading(false);
    Axios.get( 
      BACKEND_DOMAIN+`/api/course/${cid}/section/${id}`,
      config
    ).then((response)=>{
      setloading(true);
  ...
  return (
    ...
    <div className='commands container'>
      ...
      <div className='spacebtween'>
        <Link to={(section.next!=-1)?`/course/${cid}/section/${section.next}`:""} onClick={()=>setrefresh(refresh+1)}>
        <button>التالي</button>
        </Link>
        <Link to={(section.last!=-1)?`/course/${cid}/section/${section.last}`:""} onClick={()=>setrefresh(refresh+1)}>
